...
public class FlexTableUtil {
    ...
    /**
     * Copy an entire FlexTable from one FlexTable to another. Each element is copied by creating a new {@link HTML} widget by calling
    ...
    /**
     * Move an entire FlexTable from one FlexTable to another. Elements are moved by attempting to call {@link FlexTable#getWidget(int, int)} on the
     * source table. If no widget is found (because <code>null</code> is returned), a new {@link HTML} is created instead by calling
     * {@link FlexTable#getHTML(int, int)} on the source table.
     *
     * @param sourceTable
