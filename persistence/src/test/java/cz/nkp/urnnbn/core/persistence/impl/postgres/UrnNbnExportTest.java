...
public class UrnNbnExportTest extends AbstractDaoTest {
	...
	public void testSelectByCriteria_timestamps() throws Exception {
		...
		UrnNbnExportFilter filter = new UrnNbnExportFilter(begin, end, toRegistrarList(registrar), allEntityTypes(), false, false, false,
				true, true);
	...
	public void testSelectByCriteria_timestampsAndRegistrars() throws Exception {
		...
		// both registrars
		List<UrnNbnExport> selected = urnDao.selectByCriteria(CountryCode.getCode(),
				new UrnNbnExportFilter(begin, end, toRegistrarList(registrar1, registrar2), allEntityTypes(), false, false, false, true,
						true), false);
	...
	public void testSelectByCriteria_entityTypes() throws Exception {
		...
		// both registrars
		List<UrnNbnExport> selected = urnDao.selectByCriteria(CountryCode.getCode(), new UrnNbnExportFilter(begin, end,
				toRegistrarList(registrar1), allEntityTypes(), false, false, false, true, true), false);
		assertEquals(2, selected.size());
		// one registrar
		selected = urnDao.selectByCriteria(CountryCode.getCode(), new UrnNbnExportFilter(begin, end, toRegistrarList(registrar1),
				toEntityTypeList(EntityType.MONOGRAPH), false, false, false, true, true), false);
	...
	public void testSelectByCriteria_ccnbIssnIsbn() throws Exception {
		...
		DateTime end = new DateTime();
		// busyWait(2000);
		UrnNbnExportFilter noMmissingFilter = new UrnNbnExportFilter(begin, end, toRegistrarList(registrar1),
				toEntityTypeList(EntityType.MONOGRAPH), false, false, false, true, true);
		...
		UrnNbnExportFilter missingAllFilter = new UrnNbnExportFilter(begin, end, toRegistrarList(registrar1),
				toEntityTypeList(EntityType.MONOGRAPH), true, true, true, true, true);
	...
	public void testSelectByCriteria_activeDeactivated() throws Exception {
		...
		DateTime end = new DateTime();
		// after
		List<UrnNbnExport> both = urnDao.selectByCriteria(CountryCode.getCode(), new UrnNbnExportFilter(begin, end,
				toRegistrarList(registrar1), allEntityTypes(), false, false, false, true, true), false);
